...
begin
  ...
  where assignment.conversation_id = created_conversation_id
  for update;
  ...
  values (created_conversation_id, customer_id),
         (created_conversation_id, storefront_owner),
         (created_conversation_id, viewer)
  ...
  insert into public.business_conversation_assignments (
    ...
    created_conversation_id, member.storefront_id, member.id, 'open', viewer, now()
  )
  ...
  insert into public.business_conversation_assignment_events (
    ...
    created_conversation_id, member.storefront_id, existing_assignment.assignee_membership_id,
    member.id, viewer,
  ...
  insert into public.storefront_team_audit_events (
    ...
    case when existing_assignment.conversation_id is null then 'assigned' else 'reassigned' end,
    jsonb_build_object('conversationId', created_conversation_id)
  );
