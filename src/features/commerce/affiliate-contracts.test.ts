...
test("Seller Affiliate changes reuse the owned Product and ownership-checked RPC", () => {
  ...
  const migration = read(
    "../../../supabase/migrations/20260817110333_creator_commerce_progressive_onboarding_and_direct_publish.sql",
  );
  const affiliateMethod = repository.slice(
    repository.indexOf("async setCreatorPromotion"),
    repository.indexOf("async publishProduct"),
  );

  assert.match(
    affiliateMethod,
    /from\("products"\)[\s\S]*\.eq\("id", input\.productId\)/,
  );
  ...
  assert.match(
    migration,
    /where product\.id = p_product_id[\s\S]*product\.storefront_id = storefront\.id/i,
  );
...
test("Creator discovery filters Affiliate OFF while Buyer Shop remains unaffected", () => {
  ...
  const creatorQuery = creatorRepository.slice(
    creatorRepository.indexOf(
      "export async function listCreatorMarketplaceProducts",
    ),
    creatorRepository.indexOf("export async function listMyPromotions"),
  );
  const buyerQuery = shopRepository.slice(
    shopRepository.indexOf("async listProducts"),
    shopRepository.indexOf("async listStorefronts"),
  );
...
test("promotion creation preserves Seller ownership, commission snapshot and attribution history", () => {
  ...
  assert.match(
    lifecycleMigration,
    /create or replace function public\.create_creator_product_promotion/i,
  );
  ...
  assert.match(
    lifecycleMigration,
    /creator_id, seller_id, storefront_id, product_id, promotion_id, promotion_click_id/i,
  );
