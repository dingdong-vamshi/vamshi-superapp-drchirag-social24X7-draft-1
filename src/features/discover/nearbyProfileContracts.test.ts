...
test("Nearby profile migration separates bio and keeps coordinates behind discovery RPC", () => {
  const migration = readFileSync(
    new URL(
      "../../../supabase/migrations/20260825074414_nearby_profile_bio.sql",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  const publicShape =
    migration.match(
      /get_nearby_people_v2\([\s\S]*?returns table \(([\s\S]*?)\)\s*language sql/i,
    )?.[1] ?? "";
  assert.doesNotMatch(publicShape, /approximate_lat|approximate_lng/i);
  assert.match(
    migration,
    /revoke all on function public\.get_nearby_people_v2[\s\S]*from public, anon/i,
  );
});

test("Nearby client persists through Supabase RPC instead of component-only state", () => {
  const repository = readFileSync(
    new URL("./nearbyPeopleRepository.ts", import.meta.url),
    "utf8",
  );
  const screen = readFileSync(
    new URL("./NearbyPeopleScreen.tsx", import.meta.url),
    "utf8",
  );
...
test("Nearby radius uses atomic press controls without a custom PanResponder lifecycle", () => {
  const screen = readFileSync(
    new URL("./NearbyPeopleScreen.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    screen,
    /PanResponder|onTouchStart|onTouchMove|onTouchEnd/,
  );
  assert.match(screen, /accessibilityLabel="Decrease Nearby distance"/);
  assert.match(screen, /accessibilityLabel="Increase Nearby distance"/);
  assert.match(screen, /accessibilityLabel="Nearby distance radius"/);
});
