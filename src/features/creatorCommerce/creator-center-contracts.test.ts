...
test('Creator Center scopes private rows to the signed-in Creator', () => {
  ...
  assert.match(repository, /from\('creator_product_promotions'\)[\s\S]*?\.eq\('creator_id', userId\)/);
  assert.match(repository, /from\('creator_commissions'\)[\s\S]*?\.eq\('creator_id', userId\)/);
...
test('Creator/Seller and Buyer/Seller use distinct authoritative storefront conversation identities', () => {
  ...
  const chatMigration = read('../../../supabase/migrations/20260831114255_isolate_creator_seller_conversations.sql');
  ...
  assert.match(creatorRepository, /creatorCommerceChannel\s*=\s*'creator_seller'/);
  assert.match(creatorRepository, /message_payload:\s*\{\s*commerce_channel:\s*creatorCommerceChannel/);
  assert.match(creatorRepository, /\.eq\('business_context', 'creator_seller'\)/);
  assert.match(sellerRepository, /commerce_channel:\s*"creator_seller"/);
  assert.match(sellerRepository, /\.eq\("business_context", "creator_seller"\)/);
  ...
  assert.match(sellerRepository, /filter:\s*`conversation_id=eq\.\$\{conversationId\}`/);
  ...
  assert.match(chatRepository, /query\.eq\('business_context', 'creator_seller'\)/);
  assert.match(chatMigration, /unique index conversations_business_context_identity_key[\s\S]*\(storefront_id, business_customer_id, business_context\)/i);
...
test('Growth aggregate is approved-Creator-only and excludes unfinished or reversed commerce', () => {
  const migration = read('../../../supabase/migrations/20260831053649_creator_center_growth_leaderboard.sql');
  assert.match(migration, /auth\.uid\(\) is null/);
  assert.match(migration, /access\.creator_status = 'approved'/);
  assert.match(migration, /from public\.creator_commerce_access access[\s\S]*left join public\.creator_commissions/);
  assert.match(migration, /case when commerce_order\.status = 'delivered'/);
  assert.match(migration, /commerce_order\.status = 'delivered'/);
  assert.match(migration, /commission\.status in \('confirmed', 'eligible', 'payable', 'paid'\)/);
...
test('Creator Home and Discover Orders receive the same period-filtered commission source', () => {
  ...
  assert.match(screen, /same period-filtered attribution records power Creator Home and Discover/i);
});

test('Counterparty directory restricts search and conversation creation to approved Creator/Seller roles', () => {
  const migration = read('../../../supabase/migrations/20260831082001_creator_seller_counterparty_directory.sql');
  const repository = read('./creator-center-repository.ts');
  const screen = read('./CreatorCenterScreen.tsx');
  ...
  assert.match(migration, /on conflict \(storefront_id, business_customer_id\) where kind = 'business'/i);
  const isolationMigration = read('../../../supabase/migrations/20260831114255_isolate_creator_seller_conversations.sql');
  assert.match(isolationMigration, /on conflict \(storefront_id, business_customer_id, business_context\) where kind = 'business'/i);
  assert.match(migration, /revoke all[\s\S]*from public, anon/);
  assert.match(repository, /searchApprovedSellers[\s\S]*p_role: 'seller'/);
  assert.match(repository, /openApprovedSellerConversation[\s\S]*p_role: 'seller'/);
