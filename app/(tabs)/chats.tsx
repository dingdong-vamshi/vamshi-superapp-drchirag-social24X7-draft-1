...
export default function ChatsPage() {
  ...
  return <ChatScreen dataSource={repository} callAdapter={callAdapter} viewer={viewer} initialConversationId={params.conversationId} onOpenOwnProfile={() => router.push('/profile')} sharedPost={sharedPost} onShareComplete={() => router.replace('/chats')} onOpenQr={() => router.push('/profile-qr')} onBusinessSearch={() => router.push('/business-directory')} onOpenChatDetails={(conversationId) => router.push({ pathname: '/chat-details/[id]', params: { id: conversationId } })} onViewStore={(slug) => router.push({ pathname: '/store/[slug]', params: { slug } })} onViewOrder={(orderId) => router.push({ pathname: '/commerce/order/[id]', params: { id: orderId, from: 'chats' } })} onViewProfile={(userId) => router.push({ pathname: '/social-profile', params: { userId } })} />;
}
